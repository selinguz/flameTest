...
class LevelData {
  ...
  List<ObstacleData> level1() {
    ...
    level.addAll(
      obstacleRow(row: 0, item1: ObstacleType.trash, item5: ObstacleType.water),
    );

    level.addAll(
      obstacleRow(row: 1, item2: ObstacleType.trash, item5: ObstacleType.trash),
    );

    level.addAll(
      obstacleRow(row: 2, item3: ObstacleType.trash, item4: ObstacleType.trash),
    );

    level.addAll(obstacleRow(row: 4, item1: ObstacleType.trash));

    level.addAll(
      obstacleRow(row: 5, item4: ObstacleType.trash, item5: ObstacleType.trash),
    );

    level.addAll(obstacleRow(row: 7, item2: ObstacleType.trash));
  ...
  List<ObstacleData> obstacleRow({
    ...
    if (item1 != null) {
      content.add(
        ObstacleData(position: Vector2(leftSide, yPosition), type: item1),
      );
    }
    if (item2 != null) {
      content.add(
    ...
    if (item3 != null) {
      content.add(ObstacleData(position: Vector2(0, yPosition), type: item3));
    }
    if (item4 != null) {
      content.add(
        ObstacleData(
    ...
    if (item5 != null) {
      content.add(
        ObstacleData(position: Vector2(rightSide, yPosition), type: item5),
      );
    }
